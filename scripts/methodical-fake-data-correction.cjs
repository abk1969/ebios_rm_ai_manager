...
const REAL_CORRECTION_STRATEGIES = {
  HARDCODED_DATES: {
  ...
  HARDCODED_NAMES: {
    strategy: `Générateurs-${Date.now()}`,
    risk: 'MOYEN',
    testRequired: true
  },
  MOCK_VARIABLES: {
    strategy: `Services-${Date.now()}`,
    risk: 'ÉLEVÉ',
    testRequired: true
  },
  SIMULATION_COMMENTS: {
    strategy: `Commentaires-${Date.now()}`,
    risk: 'FAIBLE',
    testRequired: false
  },
  MATH_RANDOM: {
...
function applySafeCorrections(filePath) {
  ...
  try {
    ...
    content = content.replace(/\/\/.*[Ss]imulation.*$/gm, '// Données dynamiques');
    content = content.replace(/\/\/.*[Mm]ock.*$/gm, '// Données dynamiques');
    content = content.replace(/\/\/.*[Dd]emo.*$/gm, '// Données dynamiques');
