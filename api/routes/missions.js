...
// Base de données processedMissionsData
let processedMissionsData = [
  {
    id: 1,
...
router.get('/', (req, res) => {
  try {
    ...
    console.error('Get missionsData error:', error);
...
router.get('/:id', (req, res) => {
  try {
    ...
    const mission = processedMissionsData.find(m => m.id === missionId);
...
router.post('/', [
  ...
  try {
    ...
    processedMissionsData.push(newMission);
...
router.put('/:id', [
  ...
  try {
    ...
    const missionId = parseInt(req.params.id);
    const missionIndex = processedMissionsData.findIndex(m => m.id === missionId);
    ...
    res.json({
      ...
      mission: {
        ...missionsData[missionIndex],
        progress: calculateProgress(missionsData[missionIndex].workshops)
      }
    });
  } catch (error) {
