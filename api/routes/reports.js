...
router.get('/', (req, res) => {
  try {
    ...
    console.error('Get reportsData error:', error);
...
router.get('/:reportId/download', (req, res) => {
  try {
    ...
    res.json({
      success: true,
      download: {
        url: `/api/reportsData/${reportId}/file`,
...
router.delete('/:reportId', (req, res) => {
  try {
    ...
    const deletedReport = reportsData.splice(reportIndex, 1)[0];
...
router.get('/analytics/summary', (req, res) => {
  try {
    const analytics = {
      ...
      reportsByType: {},
      totalDownloads: reportsData.reduce((sum, r) => sum + (r.downloadCount || 0), 0),
      averageGenerationTime: '2.5 minutes', // Mock data
      mostPopularFormat: 'pdf'
    };
    
    // Calculate reportsData by type
    Object.keys(reportTemplates).forEach(type => {
      analytics.reportsByType[type] = reportsData.filter(r => r.type === type).length;
    });
