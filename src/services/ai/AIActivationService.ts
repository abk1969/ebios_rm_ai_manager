...
export class AIActivationService {
  ...
  private static generateAttackPathAIMetadata(data: any): AIMetadata {
    return {
      aiGenerated: true,
      confidence: (0.82 + (Date.now() % 100) / 10000),
  ...
  private static generateSecurityMeasureAIMetadata(data: any): AIMetadata {
    return {
      aiGenerated: true,
      confidence: (0.88 + (Date.now() % 100) / 10000),
      lastAnalysis: new Date().toISOString(),
      suggestions: [
    ...
    const sampleAttackPaths = [
      {
        ...
        aiMetadata: {
          aiGenerated: true,
          confidence: (0.85 + (Date.now() % 100) / 10000),
          lastAnalysis: new Date().toISOString(),
          suggestions: [
      ...
      },
      {
        ...
        aiMetadata: {
          aiGenerated: true,
          confidence: (0.78 + (Date.now() % 100) / 10000),
          lastAnalysis: new Date().toISOString(),
          suggestions: [
    ...
    const sampleSecurityMeasures = [
      {
        ...
        aiMetadata: {
          aiGenerated: true,
          confidence: (0.92 + (Date.now() % 100) / 10000),
          lastAnalysis: new Date().toISOString(),
          suggestions: [
