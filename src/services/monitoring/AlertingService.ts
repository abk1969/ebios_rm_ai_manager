/**
 * 🚨 ALERTING SERVICE - SYSTÈME D'ALERTES INTELLIGENT
 * Service d'alertes avec seuils adaptatifs et escalade automatique
 * CRITICITÉ : HIGH - Surveillance proactive de l'écosystème agentic
 */
...
export class AlertingService {
  ...
  private async initializeService(): Promise<void> {
    try {
      console.log('🚨 Initialisation Alerting Service');
      ...
      // Démarrer l'adaptation des seuils
      if (this.configuration.adaptiveThresholdsEnabled) {
        this.startAdaptiveThresholdAdjustment();
      }
      
      console.log('✅ Alerting Service initialisé');
    } catch (error) {
      console.error('❌ Erreur initialisation Alerting Service:', error);
      throw error;
    }
  }

  /**
   * Évalue une métrique et déclenche des alertes si nécessaire
   */
  async evaluateMetric(
    ...
    try {
      ...
      // Vérifier les fenêtres de maintenance
      ...
      // Vérifier les violations consécutives
      ...
      if (consecutiveViolations < threshold.consecutiveViolations) {
        console.log(`⚠️ Violation ${consecutiveViolations}/${threshold.consecutiveViolations} pour ${metricName}`);
        return;
      }
      ...
      console.error(`❌ Erreur évaluation métrique ${metricName}:`, error);
    }
  }

  /**
   * Évalue un seuil d'alerte
   */
  ...
  /**
   * Vérifie les violations consécutives
   */
  private async checkConsecutiveViolations(
    ...
    try {
      ...
      console.error('Erreur vérification violations consécutives:', error);
      return 0;
    }
  }
  ...
  private async createOrUpdateAlert(params: {
    ...
    if (existingAlertId) {
      ...
      // Ajouter à la carte des alertes actives
      this.activeAlerts.set(alert.id, alert);
      
      // Déclencher les notifications
      await this.triggerNotifications(alert);
      
      // Démarrer l'escalade si configurée
  ...
  private async updateExistingAlert(
    ...
    try {
      ...
      // Mettre à jour l'alerte en mémoire
  ...
  private async resolveAlertIfExists(
    ...
    if (alertId) {
      await this.resolveAlert(alertId, 'auto_resolved', 'Métrique revenue dans les seuils normaux');
    }
  }
  ...
  private async processNotificationQueue(): Promise<void> {
    ...
    try {
      ...
      await this.recordAlertAction(notification.alert.id, {
        ...
        timestamp: new Date(),
        details: `Notification envoyée via ${notification.channel}`,
        success: true
      });
      ...
      await this.recordAlertAction(notification.alert.id, {
        ...
        type: 'notification_sent',
        timestamp: new Date(),
        details: `Échec notification via ${notification.channel}`,
        success: false,
        error: (error as Error).message // 🔧 CORRECTION: Type assertion
      });
    }
  }
  ...
  private async sendNotification(notification: {
    ...
    if (!this.checkRateLimit(channel)) {
      throw new Error(`Rate limit dépassé pour ${channel}`);
    }
  ...
  private async sendEmailNotification(
    ...
    // Implémentation à connecter avec un service d'email
    console.log(`📧 Email envoyé à ${recipients.join(', ')}: ${subject}`);
  }
  ...
  private async sendSlackNotification(
    ...
    // Implémentation à connecter avec l'API Slack
    console.log(`💬 Slack envoyé à ${channels.join(', ')}: ${message}`);
  }
  ...
  private async sendTeamsNotification(
    ...
    // Implémentation à connecter avec l'API Teams
    console.log(`👥 Teams envoyé à ${channels.join(', ')}: ${message}`);
  }
  ...
  private async sendWebhookNotification(
    ...
    // Implémentation à connecter avec des webhooks
    console.log(`🔗 Webhook envoyé pour alerte: ${alert.id}`);
  }
  ...
  private async sendDashboardNotification(alert: Alert): Promise<void> {
    // Implémentation pour affichage temps réel dans le dashboard
    console.log(`📊 Notification dashboard pour alerte: ${alert.id}`);
  }
  ...
  private async sendResolutionNotification(
    ...
    if (template) {
      const content = {
        subject: `✅ Alerte résolue: ${alert.title}`,
        body: `L'alerte "${alert.title}" a été résolue.\n\nRaison: ${reason}\n\nDétails:\n- Métrique: ${alert.metricName}\n- Valeur actuelle: ${alert.currentValue} ${alert.unit}\n- Composant: ${alert.context.component}\n- Résolu le: ${new Date().toISOString()}`
      };
      
      console.log(`✅ Notification résolution envoyée: ${alert.id}`);
    }
  }
  ...
  /**
   * Vérifie et effectue une escalade
   */
  private async checkAndEscalate(
    ...
    try {
      ...
      if (!alert || alert.resolvedAt || alert.acknowledgedAt) {
        return; // Alerte résolue ou acquittée
      }
    ...
    for (const condition of conditions) {
      switch (condition.type) {
        ...
        case 'metric_degraded':
          // Vérifier si la métrique s'est dégradée
          // Implémentation à compléter
          break;
        case 'multiple_alerts':
          // Vérifier s'il y a plusieurs alertes similaires
          // Implémentation à compléter
          break;
      }
    }
  ...
  private async escalateAlert(alertId: string): Promise<void> {
    try {
      ...
      // Déclencher de nouvelles notifications
      await this.triggerNotifications(escalatedAlert);

      console.log(`🔺 Alerte escaladée: ${alertId} (niveau ${escalatedAlert.escalationLevel})`);
  ...
  /**
   * Démarre l'ajustement adaptatif des seuils
   */
  ...
  private async adjustAdaptiveThresholds(): Promise<void> {
    try {
      console.log('🔄 Ajustement seuils adaptatifs');
      ...
      for (const [metricName, threshold] of this.thresholds) {
        ...
        if (historicalData.length < 100) continue; // Pas assez de données
        
        // Calculer de nouveaux seuils basés sur les percentiles
        const newThresholds = this.calculateAdaptiveThresholds(historicalData);
        
        // Mettre à jour si les changements sont significatifs
        if (this.shouldUpdateThreshold(threshold, newThresholds)) {
          await this.updateThreshold(metricName, newThresholds);
          console.log(`📊 Seuils adaptatifs mis à jour pour ${metricName}`);
        }
      }
  ...
  /**
   * Obtient les données historiques d'une métrique
   */
  ...
  /**
   * Détermine si un seuil doit être mis à jour
   */
  private shouldUpdateThreshold(
    ...
    // Mettre à jour si le changement est > 10%
  ...
  private async getNotificationTemplate(
    ...
    // Implémentation simplifiée - à charger depuis la base
    const templates: Record<string, NotificationTemplate> = {
      [`${channel}_${severity}`]: {
        ...
        subject: '🚨 Alerte {{severity}}: {{alertTitle}}',
  ...
  /**
   * Génère une description d'alerte
   */
  private generateAlertDescription(
    ...
    return `La métrique "${metricName}" a atteint ${value} ${threshold.unit}, dépassant le seuil de ${thresholdValue} ${threshold.unit} pour le composant "${component}".`;
  }
  ...
  private async loadAlertThresholds(): Promise<void> {
    try {
      ...
      // Ajouter les seuils par défaut si aucun n'existe
      if (this.thresholds.size === 0) {
        await this.createDefaultThresholds();
      }
      
      console.log(`✅ ${this.thresholds.size} seuils d'alerte chargés`);
    } catch (error) {
  ...
  private async loadActiveAlerts(): Promise<void> {
    try {
      ...
      console.log(`✅ ${this.activeAlerts.size} alertes actives chargées`);
    } catch (error) {
  ...
  async getAlertStatistics(
    ...
    try {
      ...
      return {
        ...
        falsePositiveRate: 0, // Calculé dynamiquement
        ...
        topAlertingComponents: Object.entries(alertsByComponent)
          .map(([component, count]) => ({
            component,
            count,
            averageSeverity: 1 // Calculé dynamiquement
          }))
      ...
      const acknowledgedAlert = {
        ...
        actionsTaken: [
          ...alert.actionsTaken,
          {
            ...
            type: 'manual_intervention' as const,
            timestamp: new Date(),
            details: `Alerte acquittée${comment ? ': ' + comment : ''}`,
            success: true
          }
        ]
      };
  ...
  async suppressAlert(
    ...
    try {
      ...
      const suppressedAlert = {
        ...
        actionsTaken: [
          ...alert.actionsTaken,
          {
            ...
            type: 'manual_intervention' as const,
            timestamp: new Date(),
            details: `Alerte supprimée pour ${duration} minutes: ${reason}`,
            success: true
          }
        ]
      };
      ...
      console.log(`🔇 Alerte supprimée: ${alertId} jusqu'à ${suppressedUntil}`);
  ...
  async shutdown(): Promise<void> {
    console.log('🛑 Arrêt Alerting Service');
    ...
    console.log('✅ Alerting Service arrêté');
  }
}