...
const QualityMetricsPanel: React.FC<QualityMetricsPanelProps> = ({
  ...
  const calculateMetrics = async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 500 + (Date.now() % 1000)));
      
      const realMetrics: QualityMetrics = {
      ...
      const mockDetailedMetrics: MetricDetail[] = [
        {
          ...
          value: realMetrics.completenessScore,
          target: 90,
          status: realMetrics.completenessScore >= 90 ? 'excellent' : 
                  realMetrics.completenessScore >= 70 ? 'good' :
                  realMetrics.completenessScore >= 50 ? 'warning' : 'critical',
          description: 'Pourcentage de champs obligatoires remplis',
          recommendations: realMetrics.completenessScore < 90 ? [
        ...
        },
        {
          ...
          value: realMetrics.coherenceScore,
          target: 85,
          status: realMetrics.coherenceScore >= 85 ? 'excellent' : 
                  realMetrics.coherenceScore >= 70 ? 'good' :
                  realMetrics.coherenceScore >= 50 ? 'warning' : 'critical',
          description: 'Respect des liens logiques EBIOS RM',
          recommendations: realMetrics.coherenceScore < 85 ? [
        ...
        },
        {
          ...
          value: realMetrics.anssiCompliance,
          target: 95,
          status: realMetrics.anssiCompliance >= 95 ? 'excellent' : 
                  realMetrics.anssiCompliance >= 80 ? 'good' :
                  realMetrics.anssiCompliance >= 60 ? 'warning' : 'critical',
          description: 'Respect des exigences ANSSI EBIOS RM',
          recommendations: realMetrics.anssiCompliance < 95 ? [
        ...
        },
        {
          ...
          value: realMetrics.riskCoverage,
          target: 80,
          status: realMetrics.riskCoverage >= 80 ? 'excellent' : 
                  realMetrics.riskCoverage >= 65 ? 'good' :
                  realMetrics.riskCoverage >= 45 ? 'warning' : 'critical',
          description: 'Pourcentage de valeurs métier avec événements redoutés',
          recommendations: realMetrics.riskCoverage < 80 ? [
        ...
        },
        {
          ...
          value: realMetrics.aiEnhancement,
          target: 75,
          status: realMetrics.aiEnhancement >= 75 ? 'excellent' : 
                  realMetrics.aiEnhancement >= 60 ? 'good' :
                  realMetrics.aiEnhancement >= 40 ? 'warning' : 'critical',
          description: 'Utilisation des suggestions et validations IA',
          recommendations: realMetrics.aiEnhancement < 75 ? [
  ...
  const calculateANSSICompliance = (): number => {
    ...
    const assetCoverage = businessValues.length > 0
      ? (businessValues.filter((bv: any) => supportingAssets.some((sa: any) => sa.businessValueId === bv.id)).length / businessValues.length) * 100
      : 0;
    ...
    const eventCoverage = businessValues.length > 0
      ? (businessValues.filter((bv: any) => dreadedEvents.some((de: any) => de.businessValueId === bv.id)).length / businessValues.length) * 100
      : 0;
  ...
  const calculateMethodologyAdherence = (): number => {
    ...
    const hasProperLinks = businessValues.some((bv: any) =>
